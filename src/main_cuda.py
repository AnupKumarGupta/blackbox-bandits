...
def make_adversarial_examples(image, true_label, args, model_to_fool, IMAGENET_SL):
    ...
    print("Starting making adversarial examples")
    # Initial setup
    ...
    while not ch.any(total_queries > args.max_queries):
        if t % 1000 == 0:
            print("Starting {}th iteration {}".format(t+1, datetime.datetime.now().time()))
...
def main(args, model_to_fool, dataset_size):
    ...
    for i, (images, targets) in enumerate(dataset_loader):
        print("Start processing {}th batch at {}".format(i, datetime.datetime.now().time()))
        ...
        print("Ended processing {}th batch at {}".format(i, datetime.datetime.now().time()))
